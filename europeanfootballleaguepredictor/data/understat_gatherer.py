...
class Understat_Parser():
    def __init__(self, league: str, dictionary : dict):
        self.league = league
    ...
    def _replace_team_names_in_dataframe(self, data, replacing_dict):
        try:
    ...
    @logger.catch
    async def get_understat_season_to_csv(self, season: str, months_of_form: int, output_path: str, data_co_uk_path: str):
    ...
    async def get_upcoming_match_stats(self, current_season: str, months_of_form_list: int, upcoming_fixtures_path: str):
            async with aiohttp.ClientSession(cookies={'beget':'begetok'}) as session:
